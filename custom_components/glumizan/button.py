...
class GluMizanAcknowledgeButton(ButtonEntity):
    def __init__(self, coordinator, alias, grant_id):
        self.coordinator = coordinator; self.alias = alias; self.grant_id = grant_id; self._attr_unique_id = f"{DOMAIN}_{alias}_{grant_id}_acknowledge"; self._attr_name = f"GluMizan {alias} Acknowledge alert"
    @property
    def available(self):
...
class GluMizanPresenceButton(ButtonEntity):
    def __init__(self, coordinator, alias, grant_id, action):
        self.coordinator = coordinator; self.alias = alias; self.grant_id = grant_id; self.action = action; self._attr_unique_id = f"{DOMAIN}_{alias}_{grant_id}_{'leave' if action == 'end' else 'arrive'}"; self._attr_name = f"GluMizan {alias} {'I left the patient' if action == 'end' else 'I am with the patient'}"
    @property
    def available(self):
